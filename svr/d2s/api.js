/* https://github.com/dschu012/d2s-ui/blob/6c368f14a8a6bb0d955504cc025369783a99c826/api/v1/img.routes.ts */
const fs = require('fs');
...
const d2sAPI = {
  init: (props) => {
    //requires extracted d2 data directory
    d2sAPI.base = props.base || './d2d';
    d2sAPI.loadConstantData();
    d2sAPI.loadTextFiles();
    //use external express app for handling image requests
    if (props.app?.get) {
      d2sAPI.listen(props.app);
    }
    //provide external api access to handleImgReq
    if (typeof props.api === 'object') {
      props.api.d2sAPI = { handleImgReq: d2sAPI.handleImgReq };
    }
    return d2sAPI;
  },
  loadConstantData: () => {
  ...
  handleImgReq: async (req, res) => {
    try {
      //if req is a string, treat as img src
      if (typeof req === 'string') {
        //transform into express-like request
        const src = req;
        req = { params: {}, query: {} };
        [req.params.file, req.query.string] = src.split('?');
        if (!req.params.file) {
          throw new Error('d2sAPI: File not specified')
        }
        req.params.file = req.params.file.replace('.png', '');
        if (req.query.string) {
          req.query.string.split('&').forEach(param => {
            if (param.indexOf('=') > -1) {
      ...
      if (files.length === 0) {
        throw new Error(`d2sAPI: File not found ${req.params.file}.dc6`);
      }
      ...
      new jimp(width, height, async function (err, image) {
        ...
        //respond based on type of res (callback vs web)
